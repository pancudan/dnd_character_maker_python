...
def save_character_sheet(character):
    filename = get_next_filename()  # Use get_next_filename function
    with open(filename, "w") as file:
        file.write("Character Sheet:\n")
        for key, value in character.items():
            file.write(f"{key}: {value}\n")
    return filename  # Return the filename so it can be accessed outside the function
...
while True:
    ...
    saved_filename = save_character_sheet(new_character)  # Store the returned filename
